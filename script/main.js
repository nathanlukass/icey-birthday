        ...
        if (result.isConfirmed) {
            ...
            if (audio) {
                audio.play().catch((error) => {
                    console.warn("Audio tidak bisa diputar secara otomatis:", error);
                });
            }
        }
        animationTimeline(); // tetap jalankan animasi
    });
});
...
const animationTimeline = () => {
    ...
    tl.to(".container", 0.5, { visibility: "visible" })
        .from(".one", 0.5, { opacity: 0, y: 10 })
        .from(".two", 0.5, { opacity: 0, y: 10 })
        .to(".one", 0.5, { opacity: 0, y: 10 }, "+=3.5")
        .to(".two", 0.5, { opacity: 0, y: 10 }, "-=1")
